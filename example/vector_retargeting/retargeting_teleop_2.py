...
def start_retargeting(queue: multiprocessing.Queue,
    ...
    # Recorder is started only when saving begins (inside _open_save_files)
    recorder     = None
    ...
    _frame_counter = 0      # for render throttling
    # Render preview every N retargeting frames (10fps preview @ 30fps retarget)
    _RENDER_EVERY  = 3      # = ~10 fps robot preview
    ...
    def _open_save_files():
        ...
        nonlocal timestamp, video_path, recorder
        timestamp     = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_folder = os.path.join(base_dir,
                                     f"retargeting_{timestamp}")
        os.makedirs(output_folder, exist_ok=True)

        # Start HandDataRecorder now that we have a real output path
        output_csv = os.path.join(output_folder,
                                  f"teleop_data_{timestamp}.csv")
        recorder = HandDataRecorder(reply_mode=0, output_csv=output_csv,
                                    client=hand_client)
        recorder.start()
    ...
    def _close_save_files():
        ...
        nonlocal video_writer, angle_logger, log_file, recorder
    ...
    try:
        while True:
            ...
            # Only draw skeleton + encode for display on throttled frames
            # — avoids burning ~2ms on draw_skeleton every single frame
            _do_display = (_frame_counter % _RENDER_EVERY == 0)
            if _do_display or (currently_saving and video_writer is not None):
                bgr_disp = detector.draw_skeleton_on_image(
                    bgr.copy(), keypoint_2d, style="default")
            else:
                bgr_disp = bgr   # raw frame, no skeleton overlay needed

            if video_writer is not None and currently_saving:
                video_writer.write(bgr_disp)

            if _use_viewer:
                cv2.imshow("retargeting_live", bgr_disp)
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    break
            if cam_shm is not None and _do_display:
                small = cv2.resize(bgr_disp, (320, 240),
            ...
            if _use_viewer and viewer is not None:
                # Original path: viewer renders at full rate (unchanged)
                for _ in range(2):
                    viewer.render()
            elif robot_shm is not None and (_frame_counter % _RENDER_EVERY == 0):
                # Headless render throttled to ~10fps.
                # update_render() + take_picture() forces a GPU readback;
                # doing it every frame would cost 10-20ms on integrated GPUs.
                try:
                    scene.update_render()
                    cam.take_picture()
        ...
        _close_save_files()   # handles recorder, video, logs — no-op if already closed
    ...
    if camera_path == "realsense":
        ...
        try:
            while True:
                ...
                # Always-fresh: discard any stale frame sitting in queue,
                # then put the new one. Consumer always gets the latest frame.
                try:
                    queue.get_nowait()   # drain stale frame if present
                except Exception:
                    pass
                try:
        ...
        while cap.isOpened():
            ...
            if not success:
                time.sleep(0.01)
                continue
            # Always-fresh: discard stale frame, put new one
            try:
                queue.get_nowait()
            except Exception:
                pass
...
def main(
    ...
    q        = multiprocessing.Queue(maxsize=1)   # always-fresh frames
