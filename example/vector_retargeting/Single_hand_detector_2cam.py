...
class HandVisualizer:
    def __init__(self):
        ...
        self.lines.points = o3d.utility.Vector3dVector(dummy)
    ...
    def update(self, joint_pos):
        # joint_pos = joint_pos * 3.0  # scale up

        self.pcd.points = o3d.utility.Vector3dVector(joint_pos)
        self.lines.points = o3d.utility.Vector3dVector(joint_pos)
...
class SingleHandDetector:
    ...
    def detect(self, rgb):
        ...
        # print("joint_pos min/max:", joint_pos.min(), joint_pos.max())

        # #################3D Test ##################
        # joint_pos = self.parse_keypoint_3d(keypoint_3d)
        # joint_pos = joint_pos - joint_pos[0]  # center at wrist
    ...
    def detect_fused(self, rgb1, rgb2):
        ...
        Returns same signature as detect(): (num_box, joint_pos, keypoint_2d, mediapipe_wrist_rot)
        """
        # Run both detections independently
        num_box1, joint_pos1, keypoint_2d1, wrist_rot1 = self.detect(rgb1)
        num_box2, joint_pos2, keypoint_2d2, wrist_rot2 = self.detect(rgb2)
        ...
        # Build per-landmark visibility scores for each camera
        # using the same logic as your original get_landmarks_and_visibility
        vis1 = self._landmark_visibility(keypoint_2d1, W1, H1)
        vis2 = self._landmark_visibility(keypoint_2d2, W2, H2)

        # Fuse: per-landmark pick the better camera
        ...
        # For keypoint_2d (used for drawing), return whichever camera
...
if __name__ == "__main__":
    ...
    detector = SingleHandDetector(hand_type="Left", track_thumb=True)
    ...
    try:
        while True:
            ...
            num_box, joint_pos, keypoint_2d, wrist_rot = detector.detect_fused(rgb1, rgb2)
            
            if joint_pos is not None:
                viewer.update(joint_pos)
