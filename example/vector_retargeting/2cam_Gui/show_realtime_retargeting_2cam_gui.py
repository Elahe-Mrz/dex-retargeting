...
class AngleCSVLogger(object):
    ...
    def _ensure_open(self, n_cols):
        ...
        header = ["frame_capture_t", "command_sent_t", "timestamp_fused_ms"] + self.finger_names
    ...
    def write(self, frame_capture_t, command_sent_t, timestamp_fused_ms=None,
        ...
        fused_str = (f"{float(timestamp_fused_ms):.3f}"
                     if timestamp_fused_ms is not None else "")
        prefix = [f"{frame_capture_t:.6f}", f"{command_sent_t:.6f}", fused_str]
...
def _sapien_cam_to_rgb(cam):
    try:
        # Get RGBA float image [H, W, 4], range [0,1]
        ...
        print("Camera extraction error:", e)
        return None
...
def start_retargeting(
    ...
    hand_client = None
    print("use_hand", use_hand)
    print("hw_retargeting_enabled", hw_retargeting_enabled)
    if use_hand and hw_retargeting_enabled:
        try:
            ...
            print("Hand connection failed:", e)
            hand_client = None
    else:
        print("Running without hand")
    ...
    try:
        while stop_event is None or not stop_event.is_set():
                ...
                if hand_conf1 >= MIN_HAND_CONF and hand_conf2 >= MIN_HAND_CONF:
                    ...
                    if result is not None:
                        ...
                        if joint_pos is not None and (_do_display or (currently_saving and video_writer1 is not None)):
                            # Fix: Project using distortion coefficients to match the distorted HUD image
                            rvec1 = np.zeros(3, dtype=np.float64)
                            tvec1 = np.zeros(3, dtype=np.float64)
                            rvec2, _ = cv2.Rodrigues(R)
                            
                            proj1_full, _ = cv2.projectPoints(pts3D.astype(np.float64), rvec1, tvec1, K1, D1)
                            proj2_full, _ = cv2.projectPoints(pts3D.astype(np.float64), rvec2, T, K2, D2)
                            proj1 = proj1_full[:, 0, :]
                            proj2 = proj2_full[:, 0, :]
            ...
            if cam_shm is not None and _do_display:
                ...
                small = cv2.resize(disp_combined, (640, 240), interpolation=cv2.INTER_LINEAR)
            ...
            if joint_pos is not None:
                ...
                if currently_saving and angle_logger is not None:
                    angle_logger.write(
                        frame_capture_t,
                        command_sent_t,
                        timestamp_fused_ms=timestamp_fused_ms,
                        raw=hand_angles,
            ...
            if _use_viewer and viewer is not None:
                ...
                try:
                    ...
                    if rgba is not None:
                        ...
                        target_w = 480   # or 640 for bigger display
                        scale = target_w / w
                        target_h = int(h * scale)

                        small = cv2.resize(rgb, (640,240), interpolation=cv2.INTER_LINEAR)

                        # 5. Write to shared memory
                        _write_frame_to_shm(robot_shm, small)

                except Exception as e:
                    print("ROBOT RENDER ERROR:", e)
...
def main(
    ...
    """Standalone CLI entry point for 2-Camera Retargeting."""
    config_path = get_default_config_path(robot_name, retargeting_type, hand_type)
    robot_dir = Path(__file__).absolute().parent.parent.parent.parent / "assets" / "robots" / "hands"

    q1 = multiprocessing.Queue(maxsize=1)
    q2 = multiprocessing.Queue(maxsize=1)

    producer1 = multiprocessing.Process(target=camera_worker, args=(SERIAL_1, q1, None), daemon=True)
    producer2 = multiprocessing.Process(target=camera_worker, args=(SERIAL_2, q2, None), daemon=True)

    consumer = multiprocessing.Process(
        target=start_retargeting,
        args=(q1, q2, None, str(robot_dir), str(config_path), hand_type)
    )
