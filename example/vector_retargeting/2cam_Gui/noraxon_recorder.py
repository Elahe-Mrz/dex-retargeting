...
class _WideCSVGroup:
    def __init__(self, outfile_base: Path, rate_hz: float,
        ...
        self.path    = outfile_base.with_name(f"{stem}_{rate_tag}_{ts}.csv").with_suffix(".csv")
        ...
        self._last_pkt_epoch: Optional[float] = None

    def set_epoch0(self, epoch_s: float):
        self._last_pkt_epoch = float(epoch_s)
    ...
    def drain_rows(self):
        if self.strict:
            ...
            if self.epoch0 is None and self._last_pkt_epoch is not None and k > 0:
                # MR3 packet timestamps correspond to the newest sample in the batch.
                # Back-fill the first emitted row so saved CSV timestamps match the
                # per-sample timing used by the GUI buffer.
                self.epoch0 = self._last_pkt_epoch - (k - 1) * self.step
            while k > 0:
            ...
            if self.epoch0 is None and self._last_pkt_epoch is not None:
                k0 = self._min_buffer_len()
                if k0 > 0:
                    self.epoch0 = self._last_pkt_epoch - (k0 - 1) * self.step
