...
from show_realtime_retargeting_2cam_gui import camera_worker
...
# def _consumer_entry_2cam_top(q1, q2, rd, cp, use_hand, base_dir, save_event, c_shm, r_shm, hwe, ht):
#     import os
#     os.environ["NEUROCAPTURE_BASE_DIR"] = base_dir
#     from show_realtime_retargeting_2cam_gui import start_retargeting
#     start_retargeting(q1, q2, None,rd, cp, ht, use_hand,
#                       save_event=save_event, cam_shm=c_shm, robot_shm=r_shm, hw_retargeting_enabled=hwe)


def _consumer_entry_2cam_top(
    q1, q2, rd, cp, use_hand,
    base_dir, save_event, c_shm, r_shm, hwe,
    ht, base_name, grasp_type, is_guided, session_dir
):
    import os
    os.environ["NEUROCAPTURE_BASE_DIR"] = base_dir
    from show_realtime_retargeting_2cam_gui import start_retargeting
    print("ENTRY session_dir:", session_dir)
    start_retargeting(
        q1, q2, None,
        rd, cp, ht, use_hand,
        base_name=base_name,
        ...
        cam_shm=c_shm,
        robot_shm=r_shm,
        hw_retargeting_enabled=hwe,
        session_dir=session_dir   )

def _consumer_entry_top(q, rd, cp, base_dir, save_event, use_hand,cam_shm, robot_shm, hwe, ht):
    import os
    os.environ["NEUROCAPTURE_BASE_DIR"] = base_dir
    from retargeting_teleop_2 import start_retargeting
    start_retargeting(q, rd, cp, save_event=save_event,use_hand=use_hand,
...
class DataCollectorGUI:
    ...
    def __init__(self, root):
        ...
        _SHM_MAX = 1 + 5 + 640 * 240 * 3           # fits 320×240 RGB
    ...
    def _prepare_session_dir(self):
        base_dir   = self.save_dir_var.get().strip() or "data_logs"
        base_name  = self.base_name_var.get().strip() or "user"
        is_guided  = self.guided_var.get()
        grasp_type = self.gesture_var.get().strip() or "unknown"
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        mode = grasp_type if is_guided else "unguided"

        self.session_dir = os.path.join(
            base_dir,
            base_name,
            mode,
            f"session_{self.session_ts}"
        )

        os.makedirs(self.session_dir, exist_ok=True)
        print("SESSION DIR CREATED:", self.session_dir)
    ...
    def _build_left(self, parent):
        ...
        # HAND
        fhand = self._card(inner)

        self.use_hand = tk.BooleanVar(value=False)

        tk.Checkbutton(
            fhand,
            text="Connect to Hand",
            variable=self.use_hand
        ...
        self.retarg_hw_var = tk.BooleanVar(value=True) # Hardware enabled by default per request
        self.retarg_hw_chk = tk.Checkbutton(ret_top, text="Hardware control",
                       variable=self.retarg_hw_var,
                       bg=BG2, fg=FG, selectcolor=BG3, activebackground=BG2)
        self.retarg_hw_chk.pack(side="left", padx=(12, 0))

        self._lbl(fret, "Camera setup", 1)
        self.retarg_setup_var = tk.StringVar(value="2 Cameras (Async)")
        self.retarg_setup_combo = ttk.Combobox(fret, textvariable=self.retarg_setup_var,
            state="readonly", values=["1 Camera (teleop_2)", "2 Cameras (Async)"], width=20)
        self.retarg_setup_combo.grid(row=1, column=1, sticky="w", padx=6, pady=3)

        self._lbl(fret, "Robot name", 2)
        self.retarg_robot_var = tk.StringVar(value="ability")
        self.retarg_robot_entry = self._entry(fret, self.retarg_robot_var, width=18)
        self.retarg_robot_entry.grid(row=2, column=1, sticky="we", padx=6, pady=3)

        self._lbl(fret, "Retarget type", 3)
        self.retarg_type_var = tk.StringVar(value="dexpilot")
        self.retarg_type_entry = self._entry(fret, self.retarg_type_var, width=18)
        self.retarg_type_entry.grid(row=3, column=1, sticky="we", padx=6, pady=3)

        self._lbl(fret, "Hand type", 4)
        self.retarg_hand_var = tk.StringVar(value="left")
        self.retarg_hand_combo = ttk.Combobox(fret, textvariable=self.retarg_hand_var,
            state="readonly", values=["right", "left"], width=10)
        self.retarg_hand_combo.grid(row=4, column=1, sticky="w", padx=6, pady=3)

        self._lbl(fret, "Camera", 5)
        ...
        tk.Label(fret, text="(realsense / 0 / /dev/...)",
                 bg=BG2, fg=FG3, font=FONT_SMALL).grid(row=5, column=2, sticky="w")
        self.retarg_cam_entry.grid(row=5, column=1, sticky="we", padx=6, pady=3)

        tk.Label(fret, text="Saves: angles CSV + video to session folder",
                 bg=BG2, fg=ACCENT, font=FONT_SMALL
                 ).grid(row=6, column=0, columnspan=3, sticky="w", padx=6, pady=(0, 6))
        ...
        self._retarg_widgets = [
            ...
            self.retarg_hw_chk,      self.retarg_setup_combo
        ]
    ...
    def _build_right(self, parent):
        ...
        self._cam_canvas = tk.Canvas(cam_wrap, bg=BG2,
                                     highlightthickness=0, width=640, height=240)
        ...
        self._robot_canvas = tk.Canvas(robot_wrap, bg=BG2,
                                       highlightthickness=0, width=320, height=240)
        ...
        try:
            if self.emg:
                if save and self.session_dir:
                    base_dir = self.save_dir_var.get().strip() or "./Data"
                    is_guided = self.guided_var.get()
                    grasp_type = self.gesture_var.get().strip() or "Power Grasp"
                    ...
                    #     base_dir,
                    #     base_name,                 # user folder
                    #     mode,                      # grasp or unguided
                    #     f"session_{timestamp}"     # session folder
                    # )
                    path = self.emg.stop(
                        save_csv_path=os.path.join(self.session_dir, "emg"))
                        # save_csv_path=os.path.join(output_folder, "emg"))
                        # save_csv_path=os.path.join(self.session_dir, "emg")
                        # path=output_folder
                    if path:
    ...
    def _init_fsr(self, save: bool):
        ...
        try:
            self.fsr = FSRLogger(port=port, baudrate=115200,
                                 save_dir=self.session_dir, base_name="FSR")
    ...
    def start_collection(self):
        ...
        # ✅ THEN start preview (process will now get correct path)
        if not self._streaming_active():
            self.start_preview()
    ...
    def _begin_saving(self):
        ...
        if self.emg_source_var.get() == "noraxon":
            meta["noraxon"] = dict(
                ...
                strict=bool(self.nx_strict_var.get()))
        try:
            with open(os.path.join(self.session_dir, f"session_meta_{self.session_ts}.json"), "w") as fp:
                json.dump(meta, fp, indent=2)
        except Exception as e:
            self._log(f"session_meta.json error: {e}")

        # events.csv — always created for both guided and unguided
        events_path        = os.path.join(self.session_dir, f"events_{self.session_ts}.csv")
    ...
    def _start_retargeting(self):
        ...
        grasp_type = self.gesture_var.get()
        is_guided = self.guided_var.get()   
        ...
        try:
            ...
            use_2cam = getattr(self, "retarg_setup_var", None) and self.retarg_setup_var.get().startswith("2")
            if use_2cam:
                from show_realtime_retargeting_2cam_gui import start_retargeting, SERIAL_1, SERIAL_2
            else:
        ...
        hw_enabled = getattr(self, "retarg_hw_var", None) and self.retarg_hw_var.get()
        
        if use_2cam:
            
            self.retarg_queue1 = multiprocessing.Queue(maxsize=1)
            self.retarg_queue2 = multiprocessing.Queue(maxsize=1)
            
            self.retarg_producer1 = multiprocessing.Process(
                target=camera_worker,
                args=(SERIAL_1, self.retarg_queue1, self.stream_stop),
                daemon=True)
            self.retarg_producer2 = multiprocessing.Process(
                target=camera_worker,
                args=(SERIAL_2, self.retarg_queue2, self.stream_stop),
                daemon=True)
            self.retarg_consumer = multiprocessing.Process(
                target=_consumer_entry_2cam_top,
                args=(
                    self.retarg_queue1,
                    self.retarg_queue2,
                    robot_dir,
                    ...
                    self.use_hand.get(),
                    retarg_base_dir,
                    self.retarg_save_event,
                    self.retarg_cam_shm,
                    self.retarg_robot_shm,
                    hw_enabled,
                    hand_type,
                    base_name,
                    grasp_type,
            ...
            self.retarg_producer1.start()
            self.retarg_producer2.start()
            self.retarg_consumer.start()
            
            self.retarg_producer = self.retarg_producer1
            self.retarg_producer_extra = self.retarg_producer2
            ...
            self.retarg_producer = multiprocessing.Process(
                ...
                daemon=True)
            self.retarg_consumer = multiprocessing.Process(
                target=_consumer_entry_top,
                args=(self.retarg_queue, robot_dir, str(config_path),
                      retarg_base_dir, self.retarg_save_event, self.use_hand,
                      self.retarg_cam_shm, self.retarg_robot_shm, hw_enabled, hand_type,self.session_dir),
                daemon=True)
    ...
    def _stop_retargeting(self):
        ...
        procs = [self.retarg_consumer, self.retarg_producer]
        if getattr(self, 'retarg_producer_extra', None):
            procs.append(self.retarg_producer_extra)
        ...
        self.retarg_consumer = None
        if getattr(self, 'retarg_producer_extra', None):
            self.retarg_producer_extra = None

        for q_name in ['retarg_queue', 'retarg_queue1', 'retarg_queue2']:
