...
#         # --- must have both views for triangulation ---
...
#         # --- gating (CRITICAL: copied from main) ---
...
#                 joint_pos = joint_pos_new   # ❗ do NOT freeze
#             else:
#                 joint_pos = joint_pos_new
#                 self.prev_joint = joint_pos
#                 self.prev_R = R_new
...
def start_retargeting(queue: multiprocessing.Queue, robot_dir: str, config_path: str):
    ...
    try:
        while True:
            ...
            # _, joint_pos, _, _ = detector.detect(rgb1, rgb2)
            ...
            disp1 = bgr1.copy()
            disp2 = bgr2.copy()
            # joint_pos = tracker.process(bgr1, bgr2, pts1, pts2, vis1, vis2)
            if (
                pts1 is not None and pts2 is not None and
                conf1 >= MIN_HAND_CONF and conf2 >= MIN_HAND_CONF 
                # and
                # label1 == "Right" and label2 == "Right"
            ):
                joint_pos,pts3D, frame_score, final_conf, err1, err2 = tracker.process(bgr1, bgr2, pts1, pts2, vis1, vis2)
            ...
            if joint_pos is None:
                ...
                # print(f"retargeting_type: {retargeting_type}")
                # print(f"indices shape: {indices.shape}")
                # print(f"ref_value: {ref_value}")
                # print(f"ref_value range: min={ref_value.min():.4f} max={ref_value.max():.4f}")
                # print(f"joint pose: {joint_pos}")
                # RETARGETING_SCALE = 0.08  # tuned to match MediaPipe world landmark scale

                # if retargeting_type == "POSITION":

                #     ref_value = joint_pos[indices, :] * RETARGETING_SCALE
                # else:
                #     origin_indices = indices[0, :]
                #     task_indices = indices[1, :]
                #     ref_value = (joint_pos[task_indices, :] - joint_pos[origin_indices, :]) * RETARGETING_SCALE
