...
from landmarkfusion import MultiViewTracker, get_landmarks_and_visibility, TRACKED_HAND
import os
import csv
from datetime import datetime
...
calib = np.load("stereo_cam13_recalibrated.npz")
...
def start_retargeting(joint_queue: multiprocessing.Queue, image_queue: multiprocessing.Queue,  robot_dir: str, config_path: str):
    ...
    print("[SAPIEN] Creating SAPIEN viewer...")
    viewer = Viewer()
    ...
    log_file.write(f"{'Original Angles':<40} | {'Calibrated Angles':<40} ")
    log_file.write("-" * 120 + "\n")
    ...
    try:
        while True:
            try:
                ...
                bgr1, bgr2 = None, None
                try:
                    bgr1, bgr2 = image_queue.get_nowait()
                except Empty:
                    pass
            ...
            if record_video and video_writer is None and bgr1 is not None:
                h, w = bgr1.shape[:2]
            ...
            # _, joint_pos, keypoint_2d, _ = detector.detect(rgb1, rgb2, P1, P2)
            # pts1, vis1, conf1, label1, res1 = get_landmarks_and_visibility(rgb1, hands1)
            # pts2, vis2, conf2, label2, res2 = get_landmarks_and_visibility(rgb2, hands2)

            # if (
            #     pts1 is not None and pts2 is not None and
            #     label1 == TRACKED_HAND and label2 == TRACKED_HAND
            # ):
            #     joint_pos = tracker.process(rgb1, rgb2, pts1, pts2, vis1, vis2)
            # else:
            #     joint_pos = None
            ...
            try:
                while True:
                    joint_pos = joint_queue.get_nowait()
            except Empty:
                pass
            ...
            #     if isinstance(keypoint_2d, tuple):
            #         kp2d, proj1, proj2 = keypoint_2d

            #         bgr1 = detector.draw_skeleton_on_image(bgr1, kp2d, proj1)
            #         bgr2 = detector.draw_skeleton_on_image(bgr2, kp2d, proj2)
            ...
            if video_writer is not None and bgr1 is not None:
            ...
            # cv2.imshow("realtime_retargeting_demo_cam2", bgr2)
...
#         # Initialize RealSense pipeline for 2 cameras
...
#                 frames1 = pipeline1.poll_for_frames()
...
def produce_joint(joint_queue, image_queue, camera_path="realsense"):

    # --- init cameras ---
    print(f"[CAMERA] Starting camera pipelines for devices {SERIAL_1} and {SERIAL_2}...")
    pipeline1 = rs.pipeline()
    config1 = rs.config()
    config1.enable_device(SERIAL_1)
    config1.enable_stream(rs.stream.color, 640, 480, rs.format.bgr8, 30)
    ...
    pipeline1.start(config1)
    pipeline2.start(config2)
    print("Camera pipelines started successfully.")

    # --- init mediapipe + tracker (MOVE HERE) ---
    mp_hands = mp.solutions.hands
    ...
    try:
        while True:
            ...
            # --- tracking here ---
            pts1, vis1, conf1, label1, _ = get_landmarks_and_visibility(rgb1, hands1)
            pts2, vis2, conf2, label2, _ = get_landmarks_and_visibility(rgb2, hands2)

            if (
                pts1 is not None and pts2 is not None and
                label1 == TRACKED_HAND and label2 == TRACKED_HAND
            ):
                joint_pos = tracker.process(img1, img2, pts1, pts2, vis1, vis2)
            else:
                joint_pos = None

            # --- send joint (real-time critical) ---
            if joint_queue.full():
                try:
                    joint_queue.get_nowait()
                except:
                    pass

            joint_queue.put_nowait(joint_pos)


            # --- send images (best effort only) ---
            if image_queue.full():
                try:
            ...
            image_queue.put_nowait((img1, img2))
...
def main(
    ...
    joint_queue = multiprocessing.Queue(maxsize=1)     # critical
    image_queue = multiprocessing.Queue(maxsize=2)     # small buffer OK
    # producer_process = multiprocessing.Process(
    #     target=produce_frame, args=(queue, camera_path)
    # )
    producer_process = multiprocessing.Process(
        target=produce_joint,
        args=(joint_queue, image_queue, camera_path)
    )

    consumer_process = multiprocessing.Process(
        target=start_retargeting,
        args=(joint_queue, image_queue, str(robot_dir), str(config_path))
    )
